...
+ (id)stepToLongTapViewWithAccessibilityLabel:(NSString *)label value:(NSString *)value traits:(UIAccessibilityTraits)traits;
{
    NSString *description = nil;
    if (value.length) {
        description = [NSString stringWithFormat:@"Long Tap view with accessibility label \"%@\" and accessibility value \"%@\"", label, value];
    } else {
        description = [NSString stringWithFormat:@"Long Tap view with accessibility label \"%@\"", label];
    }
    ...
    return [self stepWithDescription:description executionBlock:^(KIFTestStep *step, NSError **error) {
        ...
        [view longTapAtPoint:tappablePointInElement withDelay:2.0 andCompletion:^{
            complete = YES;
        }];
        KIFTestWaitCondition(complete, error, @"Waiting for long press.");
...
{
    return [self stepToLongTapViewWithAccessibilityLabel:label value:nil traits:UIAccessibilityTraitNone];
}
